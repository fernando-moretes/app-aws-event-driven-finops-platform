...
Connect this repository to Vercel and expose it on `finops.moretes.com` via
Cloudflare DNS.

## 1. Create the Vercel project

1. Sign in at https://vercel.com with the GitHub account that owns this repo.
2. Click **Add New… → Project** and import `fernandofatech/aws-event-driven-finops-platform`.
...
```bash
gh secret set VERCEL_TOKEN      --body "<token>" -R fernandofatech/aws-event-driven-finops-platform
gh secret set VERCEL_ORG_ID     --body "<org-id>" -R fernandofatech/aws-event-driven-finops-platform
gh secret set VERCEL_PROJECT_ID --body "<project-id>" -R fernandofatech/aws-event-driven-finops-platform
```

## 4. Attach the custom subdomain

In Vercel: **Settings → Domains → Add** → `finops.moretes.com`.
