...
const hrefs = [...html.matchAll(/href="([^"]+)"/g)].map((match) => new URL(match[1], "https://finops.moretes.com/"));
const metas = [...html.matchAll(/<meta\s+([^>]+)>/g)].map((match) => match[1]);
const links = [...html.matchAll(/<link\s+([^>]+)>/g)].map((match) => match[1]);

const hasRepoLink = hrefs.some((url) => url.protocol === "https:" && url.hostname === "github.com" && url.pathname === "/fernandofatech/aws-event-driven-finops-platform");
...
const hasCanonical = links.some((attrs) => attrs.includes('rel="canonical"') && attrs.includes('href="https://finops.moretes.com/"'));
...
const required = [
  ...
  [html.includes("AWS Event-Driven FinOps Platform"), "index.html must include the project title"],
  ...
  [robotLines.has("Sitemap: https://finops.moretes.com/sitemap.xml"), "robots.txt must point to sitemap"],
  [sitemapLocs.some((url) => url.href === "https://finops.moretes.com/"), "sitemap.xml must include canonical domain"],
